...
#[inline]
pub fn text_style_at_char_custom(
    theme: &Theme,
    cursor: &Cursor,
    char_index: usize,
    focused: bool,
    ...
    if cursor.range.contains(&char_index) {
        ...
        let (foreground, bold, underline) = if is_error {
            (Foreground(gruvbox::BRIGHT_RED), false, true)
        } else if scope.starts_with("constant") {
            ...
            || scope.starts_with("primitive_type")
        {
            ...
            (Foreground(gruvbox::LIGHT4), false, false)
        } else if scope.starts_with("comment") {
